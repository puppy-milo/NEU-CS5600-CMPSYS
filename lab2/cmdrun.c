...
cmd_exec(command_t *cmd, int *pass_pipefd)
{
	...
	if(0 == pid){
		...
		if(cmd->redirect_filename[0]){
			int fdin = open(cmd->redirect_filename[0], O_RDONLY);
			dup2(fdin, STDIN_FILENO);
			close(fdin);
		}
		if(cmd->redirect_filename[1]){
			int fdout = open(cmd->redirect_filename[1], O_WRONLY | O_CREAT | O_TRUNC, 0666);
			dup2(fdout, STDOUT_FILENO);
			close(fdout);
		}
		if(cmd->redirect_filename[2]){
			int fderr = open(cmd->redirect_filename[2], O_WRONLY | O_CREAT | O_TRUNC, 0666);
		...
		if(cmd->subshell){
			if(0 == cmd_line_exec(cmd->subshell)){
				exit(0);
			}
			exit(5);
		}
		if(cmd->argv[0] && 0 == strcmp(cmd->argv[0], "cd")){
			if(cmd->argv[1]==0 || cmd->argv[2]!=0){
		...
		if(cmd->argv[0] && 0 == strcmp("exit", cmd->argv[0])){
			if(0 != cmd->argv[2]){
				write(STDERR_FILENO,"exit: Syntax error! Wrong number of arguments!\n",47);
				exit(1);
			}
			if(!cmd->argv[1] || 0 == strcmp("0", cmd->argv[1])){
				exit(0);
			}
			int ext_code = strtol(cmd->argv[1], NULL, 10);
			if(ext_code) {
				exit(ext_code);
			}
			perror("exit");
		...
		if(cmd->argv[0] && 0 == strcmp("our_pwd", cmd->argv[0])){
			if(0 != cmd->argv[1]){
				write(STDERR_FILENO,"pwd: Syntax error! Wrong number of arguments!\n",46);
				exit(1);
			}
			char* pathname = getcwd(0, 1024);
			if(NULL == pathname){
				perror("pwd");
				exit(1);
			}
		...
		*pass_pipefd = STDIN_FILENO;
		...
		if(cmd->argv[0] && 0 == strcmp(cmd->argv[0], "exit")){
			if(0 != cmd->argv[2]){
				return pid;
			}
			waitpid(pid, 0, 0);
...
cmd_line_exec(command_t *cmdlist)
{
	...
	while (cmdlist) {
		...
		/* Your code here */
		pid_t pid = -1;
		if((pid = cmd_exec(cmdlist, &pipefd))< 0){
			abort();
		}
		switch(cmdlist->controlop){
			...
			case CMD_AND:
				waitpid(pid, &wp_status, 0);
				if(WIFEXITED(wp_status) && 0 == WEXITSTATUS(wp_status)){
					cmd_status = 0;
					break;
				}
				goto done;
